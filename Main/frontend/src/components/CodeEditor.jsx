...
export default function CodeEditor({
  ...
  return (
    <section className="glass-card flex min-h-0 flex-col p-4 sm:p-5">
      ...
      <div className="group relative h-[260px] sm:h-[320px] md:h-[380px] lg:h-[430px] overflow-hidden rounded-lg border border-neon-border bg-editor transition focus-within:border-neon-mint focus-within:shadow-neon">
        ...
        <div className="pointer-events-none absolute inset-y-0 left-0 w-12 overflow-hidden py-3">
          {Array.from({ length: lines }).map((_, idx) => {
            ...
            return (
              <div
                key={lineNo}
                className="h-6 text-center font-code text-xs leading-6 text-text-muted"
              >
                {lineNo}
              </div>
            )
